    ...
    <div class="tracking-page">
      ...
      <div class="orders-list">
        <div class="empty-state" v-if="filteredOrders.length === 0">
          ...
          <RouterLink to="/products" class="btn btn-primary" id="browse-btn-tracking">Browse Products</RouterLink>
        </div>
        ...
        <div v-for="order in filteredOrders" :key="order.id" class="order-card card">
          ...
          <div class="tracking-steps">
            <div v-for="(step, i) in order.trackingSteps" :key="step.status" class="track-step" :class="{ done: step.done, current: step.done && (i === order.trackingSteps.length - 1 || !order.trackingSteps[i+1]?.done) }">
          ...
          <div class="order-footer">
            ...
            <div class="order-actions">
              ...
              <span class="meet-loc">📍 {{ order.meetingLocation }}</span>
            </div>
          </div>
        </div>
      </div>
...
.item-thumb { width: 40px; height: 40px; background: var(--bg-page); border-radius: var(--radius-md); display: flex; align-items: center; justify-content: center; font-size: 1.2rem; }
...
.track-step.current .step-dot { box-shadow: 0 0 0 3px rgba(22,163,74,.2); }
.step-line {
  position: absolute;
  top: 7px; left: 50%;
...
.order-total-price { font-size: 18px; font-weight: 700; color: var(--text-primary); }
.order-actions { display: flex; gap: 12px; font-size: 12px; color: var(--text-secondary); }
